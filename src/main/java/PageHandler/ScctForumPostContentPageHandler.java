 ...
 */
public class ScctForumPostContentPageHandler extends PageHandler {
    private String name = "";
//    private PostList postList;

    public ScctForumPostContentPageHandler() {
        this.onInit();
    }
    public ScctForumPostContentPageHandler(SessionHandler sessionHandler) {
            ...
            if (this.urlInfo.title != null);
            {
                ...
                try {
                    ...
                    while (flag) {
                        ...
                        as.setNo(pointer+"");
                        if (browser.find("xpath","/html/body/table[2]/tbody/tr/td/table/tbody/tr/td/table[1]/tbody/tr/td[1]/a[1]") ==null)
                        {
                            browser.goBack();
                            Thread.sleep(5000);
                        ...
                        Thread.sleep(1000);
                        //题目截屏
//                        String filename = "/Users/lee/Desktop/ocr/qizhongji/input/" + pointer + ".png";
...
//                        }
                        //题目类型
                        String asType = browser.find("xpath","//*[@id=\"top\"]/table/tbody/tr[1]/td/span[1]").getText();
                        as.setType(asType);
                        //题干
                        WebElement quElement = browser.find("xpath","//*[@id=\"top\"]/table/tbody/tr[1]/td/span[2]");
                        String quFont = quElement.getAttribute("class");
                     //   TODO: 题干处理
//                        as.setQuestion(dict.getFromdict(quElement.getText(),quFont));
                        System.out.println(quElement.getText());
                        as.setQuestion(contentDict.getFromdict(quElement.getText(),quFont));
//                        as.getQuestion().replace("@","\\u");
                        //题干图片处理
//                        System.out.println(as.getQuestion().replace("@","\\u"));
                        try {
                            if (quElement.findElement(By.tagName("img")) != null) {
                                String imag = quElement.findElement(By.tagName("img")).getAttribute("src");
                        ...
                        //*[@id="box1"]/span
                        WebElement answer = browser.find("xpath", "//*[@id=\"box1\"]/span");
                        String answerType = answer.getAttribute("class");
                        //TODO 答案处理
                        as.setAnswer(dict.getFromdict(answer.getText(),answerType));
                        //下一题
                        browser.click("xpath","/html/body/table[2]/tbody/tr/td/table/tbody/tr/td/table[1]/tbody/tr/td[1]/a[5]");
                        Thread.sleep(1000);
...
//                        }
                ...
                }catch (Exception e)
                {
                    ...
                    String pathnameT = "D:\\Desktop\\target\\" + ScctForumSessionHandler.Sname +".txt";
                    String pathnameE = "D:\\Desktop\\target\\" + ScctForumSessionHandler.Sname +".xls";
                    System.out.println(pathnameE);
                    writeObjectToFile(result,pathnameT);
                    saveData( result,pathnameE);
                }
...
//    }
    ...
    {
        IPageHandler ph = new ScctForumPostContentPageHandler(sessionHandler);
        return ph;
    }
...
//            }
    ...
    public static void saveData(List<AnswerBean> as,String path)  {
        String knowlageType = ScctForumSessionHandler.Sname;
        // 创建工作薄
        ...
        for (int row = 0; row < as.size(); row++) {
            try {
                ...
                if (as.get(row).getImage()!=null){
                    rows.createCell(1).setCellValue(as.get(row).getQuestion() + as.get(row).getImage());
                }else {
                    rows.createCell(1).setCellValue(as.get(row).getQuestion());
                }

                for (int col = 1; col <= as.get(row).getOption().size(); col++) {
                    rows.createCell(col + 1).setCellValue(as.get(row).getOption().get(col - 1));
                }
                rows.createCell(12).setCellValue(as.get(row).getAnswer());
                rows.createCell(13).setCellValue(knowlageType);
                rows.createCell(14).setCellValue(as.get(row).getType());
                rows.createCell(15).setCellValue(as.get(row).getImage());
                label++;
            }catch (Exception e){
    ...
    public static void checkerror(Browser browser){
        try {
            String asType = browser.find("xpath","//*[@id=\"top\"]/table/tbody/tr[1]/td/span[1]").getText();
        }
        catch (Exception e)
